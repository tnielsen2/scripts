...
echo -------------------------------
#Loop through the orphaned snapshots
for SNAPSHOT in $SNAPSHOTS
do
echo Snapshot ID $SNAPSHOT in $REGION is older than 14 days. Deleting.
aws ec2 describe-snapshots --filters Name='snapshot-id',Values=$SNAPSHOT | grep 'StartTime'
done
